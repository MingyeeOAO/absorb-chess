...
#         self.en_passant_target = None  # (row, col) of pawn that can be captured en passant
#         self.promotion_pending: Optional[Dict] = None  # {'row': int, 'col': int, 'color': Color}
#         self.valid_moves = {}  # Dictionary to store valid moves for each piece, format: {(row, col): [(target_row, target_col), ...]}
#         self._initialize_board()
...
#                     valid_moves = []
#                     for ability in piece.abilities:
#                         valid_moves.extend(self.get_valid_moves_for_ability((row, col), ability))
#                     if valid_moves:
#                         self.valid_moves[(row, col)] = valid_moves
...
#         # Check if move is valid according to chess rules and doesn't put own king in check
#         if not self._is_valid_move(piece, from_pos, to_pos):
#             print("Move is invalid according to chess rules or would put own king in check")
#             return False
...
#         # Calculate valid moves for the new turn before making the move
#         self.valid_moves.clear()
...
#         # Make a temporary move and check if it puts own king in check
#         captured_piece = self.get_piece_at(to_row, to_col)
#         self.board[to_row][to_col] = piece
#         self.board[from_row][from_col] = None
#         original_position = piece.position
...
#         if king_in_check:
#             print("Move would put own king in check")
#             return False
...
#                             if (to_row, to_col) == (row, col):
#                                 continue
#                             # Check all abilities of the piece
#                             for ability in piece.abilities:
#                                 # Try the move
#                                 if self._is_valid_move(piece, (row, col), (to_row, to_col)):
#                                     valid_moves.append((to_row, to_col))
#                                     break  # No need to check other abilities if one works
#                     if valid_moves:
#                         moves[(row, col)] = valid_moves
#         return moves
